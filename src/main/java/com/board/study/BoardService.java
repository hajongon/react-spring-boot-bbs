...
import com.board.study.db.BoardMapper;
import com.board.study.dto.BoardSaveDto;
import com.board.study.util.Header;
import com.board.study.util.Pagination;
...
public class BoardService {
    // boardMapper 필드는 게시판 데이터와 상호작용하기 위한 MyBatis Mapper 인터페이스
    private final BoardMapper boardMapper;

    // 게시글 목록 조회 서비스 메서드
    // 페이지 번호, 페이지 크기, 검색 조건을 받아와서, MyBatis를 사용하여 데이터베이스에서 게시글 목록을 조회하고 페이징 정보를 계산하여 반환
    ...
    Header<List<BoardEntity>> getBoardList(int page, int size, Search search) {
        ...
        Pagination pagination = new Pagination(
                boardMapper.getBoardTotalCount(paramMap),  // 8.1. 전체 게시글 수를 조회하여 전달
